...
FString UMassBattleEditorMCPApi::MCP_DuplicateClassAsset(const FString& SourceClassPath, const FString& NewClassName, const FString& PackagePath)
{
	...
	UBlueprint* SourceBlueprint = Cast<UBlueprint>(SourceClass->ClassGeneratedBy);
	if (!SourceBlueprint)
	{
		return MakeErrorJson(FString::Printf(TEXT("Source class is not generated by a Blueprint: %s"), *SourceClassPath));
	}

	const FString TargetPackageName = PackagePath / NewClassName;
	const FString TargetObjectPath = FString::Printf(TEXT("%s.%s"), *TargetPackageName, *NewClassName);
	if (UBlueprint* ExistingBlueprint = FindObject<UBlueprint>(nullptr, *TargetObjectPath))
	{
		FKismetEditorUtilities::CompileBlueprint(ExistingBlueprint);
		TSharedPtr<FJsonObject> ExistingResult = MakeShared<FJsonObject>();
		ExistingResult->SetBoolField(TEXT("success"), ExistingBlueprint->GeneratedClass != nullptr);
		ExistingResult->SetBoolField(TEXT("reused_existing"), true);
		if (ExistingBlueprint->GeneratedClass)
		{
			ExistingResult->SetStringField(TEXT("class_path"), ExistingBlueprint->GeneratedClass->GetPathName());
		}
		else
		{
			ExistingResult->SetStringField(TEXT("error"), TEXT("Existing target Blueprint has no generated class"));
		}

		FString ExistingOutput;
		TSharedRef<TJsonWriter<>> ExistingWriter = TJsonWriterFactory<>::Create(&ExistingOutput);
		FJsonSerializer::Serialize(ExistingResult.ToSharedRef(), ExistingWriter);
		return ExistingOutput;
	}

	FAssetRegistryModule& AssetRegistryModule = FModuleManager::LoadModuleChecked<FAssetRegistryModule>(TEXT("AssetRegistry"));
	if (const FAssetData ExistingAsset = AssetRegistryModule.Get().GetAssetByObjectPath(FSoftObjectPath(TargetObjectPath)); ExistingAsset.IsValid())
	{
		UBlueprint* ExistingBlueprint = Cast<UBlueprint>(ExistingAsset.GetAsset());
		if (!ExistingBlueprint || !ExistingBlueprint->GeneratedClass)
		{
			return MakeErrorJson(FString::Printf(TEXT("Target asset already exists and is not a valid Blueprint: %s"), *TargetObjectPath));
		}
		TSharedPtr<FJsonObject> ExistingResult = MakeShared<FJsonObject>();
	...
	UBlueprint* NewBlueprint = Cast<UBlueprint>(AssetToolsModule.Get().DuplicateAsset(NewClassName, PackagePath, SourceBlueprint));
	if (NewBlueprint)
	{
		FKismetEditorUtilities::CompileBlueprint(NewBlueprint);
	}
	UClass* NewClass = NewBlueprint ? NewBlueprint->GeneratedClass : nullptr;
...
FString UMassBattleEditorMCPApi::MCP_GetApiStatus()
{
	...
	AddTool(TEXT("MCP_EditorInspectActorAssembly"),
		TEXT("检查 Actor 的模块化骨骼、静态武器、材质与绑定骨骼，不写资产"),
	...
	AddTool(TEXT("MCP_EditorApplyCreateVatUnitFromActor"),
		TEXT("把 Actor 身体、模块化部件和武器组装成可动画 SkeletalMesh，再生成 VAT 单位"),
		TEXT("SpecJson, bSaveAssets"),
		TEXT("unit_editor.actor"));
