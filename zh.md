...
| 分支 | Unreal Engine | 支持范围 |
| --- | --- | --- |
| `main` | UE 5.8 | 主分支，提供完整 MCP 功能，包括 Niagara MCP 与 Batch FX MCP。 |
| `5.7` | UE 5.7 | 维护基础 MCP 兼容性；受旧版 UE Niagara 编辑器 API 限制，不暴露 Niagara MCP 与 Batch FX MCP。 |
| `5.6` | UE 5.6 | 维护基础 MCP 兼容性；受旧版 UE Niagara 编辑器 API 限制，不暴露 Niagara MCP 与 Batch FX MCP。 |

如需为 UE 5.6/5.7 定制批处理能力，可联系 QQ `3440602831` 付费外包。

> 当前是 UE 5.6 兼容分支：保留基础 MCP 工作流，但不会注册 Niagara MCP 与 Batch FX MCP。下文保留的 Niagara 与批处理制作内容仅用于说明 UE 5.8 `main` 的完整能力。
