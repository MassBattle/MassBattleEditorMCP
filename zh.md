...
**当前分支：`main`（Unreal Engine 5.8）。** 这是主分支，包含完整的开源 MCP 功能。

| 分支 | Unreal Engine | 基础 MCP | Niagara MCP | Batch FX MCP | 获取方式与支持政策 |
| --- | --- | :---: | :---: | :---: | --- |
| **`main`（当前）** | UE 5.8 | 已包含 | 已包含 | 已包含 | 上述功能随开源分支提供，不需要订阅。 |
| `5.7` | UE 5.7 | 已包含 | 未包含 | 未包含 | 基础 MCP 保持开源免费；旧引擎批处理能力需要付费定制或订阅技术支持。 |
| `5.6` | UE 5.6 | 已包含 | 未包含 | 未包含 | 基础 MCP 保持开源免费；旧引擎批处理能力需要付费定制或订阅技术支持。 |

每个分支已有的开源代码都可以免费使用，不需要订阅。只有在 UE 5.6/5.7 上需要 Niagara MCP、Batch FX MCP 或持续维护批处理兼容性时，才需要联系 QQ `3440602831` 购买技术支持订阅或外包定制。

需求表述模板：`我使用 UE 5.6/5.7，需要 Niagara MCP 和/或 Batch FX MCP 兼容，请评估适合我项目的付费技术支持订阅或外包实现方案。`
