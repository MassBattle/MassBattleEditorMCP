...
| 分支 | Unreal Engine | 基础 MCP | Niagara MCP | Batch FX MCP | 批处理特效方案 |
| --- | --- | :---: | :---: | :---: | --- |
| `main` | UE 5.8 | 已包含 | 已包含 | 已包含 | 可通过 MCP 与 AI 制作，也可把源特效素材交给作者定制。 |
| **`5.7`（当前）** | UE 5.7 | 已包含 | 未包含 | 未包含 | 把源特效素材和需求交给作者，由作者定制批处理特效。 |
| `5.6` | UE 5.6 | 已包含 | 未包含 | 未包含 | 把源特效素材和需求交给作者，由作者定制批处理特效。 |
...
需求表述模板：`我使用 UE 5.7，以下源特效素材需要转换为 MassBattle 批处理特效：【素材与效果要求】。请针对这个引擎版本定制并交付批处理特效。`
