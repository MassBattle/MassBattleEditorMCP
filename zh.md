...
**当前分支：`5.6`（Unreal Engine 5.6）。** 这是兼容分支，包含开源的基础 MCP 工作流，但不包含 Niagara MCP 与 Batch FX MCP。

| 分支 | Unreal Engine | 基础 MCP | Niagara MCP | Batch FX MCP | 获取方式与支持政策 |
| --- | --- | :---: | :---: | :---: | --- |
| `main` | UE 5.8 | 已包含 | 已包含 | 已包含 | 当前主要验证分支。 |
| `5.7` | UE 5.7 | 已包含 | 未包含 | 未包含 | 兼容分支，仅保证表中已经验证的功能范围。 |
| **`5.6`（当前）** | UE 5.6 | 已包含 | 未包含 | 未包含 | 兼容分支，仅保证表中已经验证的功能范围。 |
...
需求表述模板：`我使用 UE 5.6，因为版本兼容无法使用【功能名称】，希望评估在目标版本或更高 UE 版本中的定制调整。我了解这是一次性定制，费用用于覆盖 ChatGPT Pro ×20 的订阅成本。`
